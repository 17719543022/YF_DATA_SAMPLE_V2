	...
	: CDialogEx(IDD_DIALOG_DIALOG, pParent)
{
	...
	m_bButtonContinuousClicked = FALSE;
}
...
void CDialogDlg::DoDataExchange(CDataExchange* pDX)
{
	...
	DDX_Control(pDX, IDC_BUTTON_CONTINUOUS, m_buttonContinuous);
}
...
BEGIN_MESSAGE_MAP(CDialogDlg, CDialogEx)
	...
	ON_WM_TIMER()
	ON_BN_CLICKED(IDC_BUTTON_QUERY, &CDialogDlg::OnBnClickedButtonQuery)
	ON_BN_CLICKED(IDC_BUTTON_DL_NUM36, &CDialogDlg::OnBnClickedButtonDlNum36)
	ON_BN_CLICKED(IDC_BUTTON_DL_NUM18, &CDialogDlg::OnBnClickedButtonDlNum18)
	ON_BN_CLICKED(IDC_BUTTON_CONTINUOUS, &CDialogDlg::OnBnClickedButtonContinuous)
END_MESSAGE_MAP()
...
void CDialogDlg::OnBnClickedButtonAdcSample()
{
	...
	if (m_bButtonADCSampleClicked == FALSE)
	{
		...
		m_buttonContinuous.EnableWindow(FALSE);

		m_pThread = AfxBeginThread((AFX_THREADPROC)PerformADCSampling, (LPVOID)this);
	}
	else
	{
		SwitchContinuousToQuery();

		m_bButtonADCSampleClicked = FALSE;

		m_buttonADCSample.SetWindowText("正式启动");

		m_buttonContinuous.EnableWindow(TRUE);
...
void CDialogDlg::SwitchContinuousToQuery()
{
	CString strOutData = m_strEndPointEnumerate0x02;
	TCHAR* pEnd;
	BYTE outEpAddress = 0x0;
	...
	epBulkOut->XferData(bufferOutput, totalOutTransferSize);
...
void CDialogDlg::OnBnClickedButtonDlNum18()
{
	CString strOutData = m_strEndPointEnumerate0x02;
	TCHAR* pEnd;
	BYTE outEpAddress = 0x0;
...
void CDialogDlg::OnBnClickedButtonDlNum36()
{
	CString strOutData = m_strEndPointEnumerate0x02;
	TCHAR* pEnd;
	BYTE outEpAddress = 0x0;
...
DWORD WINAPI CDialogDlg::ConfigContinuousSampling(LPVOID lParam)
{
	CDialogDlg* pThis = (CDialogDlg*)lParam;
	...
	long nCount = 0;
	FILE* fp = NULL;
	while (pThis->m_bButtonContinuousClicked == TRUE)
	{
		if ((fp == NULL) && (pThis->m_bButtonContinuousClicked == TRUE))
		{
			char fileName[60] = { 0 };
			fp = fopen("../samples/data.txt", "w");
		}
		...
		{
			nCount = 0;
			if ((pThis->m_bButtonContinuousClicked == FALSE) && (fp != NULL))
			{
				fclose(fp);
				fp = NULL;
			}
...
void CDialogDlg::OnBnClickedButtonContinuous()
{
	writeIndex = 0;
	
	if (m_bButtonContinuousClicked == FALSE)
	{
		m_bButtonContinuousClicked = TRUE;
	
		m_buttonContinuous.SetWindowText("问询上传");
	
		m_buttonADCSample.EnableWindow(FALSE);
	
		m_pThread = AfxBeginThread((AFX_THREADPROC)ConfigContinuousSampling, (LPVOID)this);
	}
	else
	{
		SwitchContinuousToQuery();
		...
		m_buttonContinuous.SetWindowText("自动上传");

		m_buttonADCSample.EnableWindow(TRUE);
...
void CDialogDlg::OnBnClickedButtonQuery()
{
	...
	while (nINCount < QUEUE_SIZE)
	{
		...
		m_edtQuery.SetWindowText(strBytes);
